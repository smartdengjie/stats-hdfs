...
public class BaileyBorweinPlouffe extends Configured implements Tool {
	...
	out.println();
	// final FileSystem fs = FileSystem.get(conf);// 分布式
	final FileSystem fs = new Path(workingDir, "part-r-00000").getFileSystem(conf);// 伪分布式
	...
	// 16是控制圆周率的精度，必须是4的整数倍
	args = new String[] { "1", "16", "1", String.format(ConfigUtils.HDFS.WORDCOUNT_OUT, random) };
